...
import {
  buildCumulative, keyToPair, toGlobal, surahRange,
  hizbRange, intersects, ayahRangeForSurahIntersection
} from "@/utils/quran-structure";

// pak de eerst 60 hizb 
async function fetchHizbStarts() {
  // try cache
  const cached = typeof window !== "undefined" && localStorage.getItem("hizbStarts");
  if (cached) return JSON.parse(cached);

  const starts = [];
  for (let h = 1; h <= 60; h++) {
    const r = await fetch(`https://api.quran.com/api/v4/verses/by_hizb/${h}?fields=verse_key`);
...
export function useQuranRelations() {
  ...
  const surahItems = useMemo(
    () => chapters.map(c => ({ value: String(c.id), label: `${c.id}. ${c.name_simple}` })),
    [chapters]
  );
  const hizbItemsAll = useMemo(
    () => Array.from({ length: 60 }, (_, i) => ({ value: String(i + 1), label: `Hizb ${i + 1}` })),
    []
  );
  ...
  function hizbsForSurah(surahId) {
    ...
    for (let h = 1; h <= 60; h++) {
      ...
      if (intersects([g1, g2], hr)) items.push({ value: String(h), label: `Hizb ${h}` });
    }
    return items;
  }
  ...
  function surahsForHizb(hizb) {
    ...
    return chapters
      ...
      .map(c => ({ value: String(c.id), label: `${c.id}. ${c.name_simple}` }));
  }
  ...
  function ayahsFor(surahId, maybeHizb) {
    ...
    if (!maybeHizb) {
      return Array.from({ length: versesCountBySurah[s] }, (_, i) => ({ value: String(i + 1), label: `Ayah ${i + 1}` }));
    }
    ...
    const [a1, a2] = ar;
    const items = [];
    for (let a = a1; a <= a2; a++) items.push({ value: String(a), label: `Ayah ${a}` });
    return items;
  }
