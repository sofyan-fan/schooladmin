...
exports.create_roster = async (req, res) => {
  try {
    const { class_id, subject_id, teacher_id, days } = req.body;
    ...
    res.status(201).json({ message: 'Roster created', newRoster });
  } catch (error) {
...
exports.update_roster = async (req, res) => {
  try {
    const { id } = req.params;
    const { day_of_week, start_time, end_time, subject_id, class_id, teacher_id } = req.body;
    ...
    res.status(200).json({ message: 'Roster updated', updatedRoster });
  } catch (error) {
